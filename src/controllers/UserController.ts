...
class UserController {
  ...
  public async login(req: Request, res: Response): Promise<Response> {
    ...
    if (user && user.iduser) {
      ...
      return res.json({ error: "Dados de login não condizem" });
    }
    else {
  ...
  public async create(req: Request, res: Response) {
      ...
      if (!name || name.trim().length == 0) {
          return res.json({ error: "Digite o nome do usuário" });
      }
      if (!mail || mail.trim().length == 0) {
          return res.json({ error: "Digite o e-mail do usuário" });
      }
      if (!password || password.trim().length == 0) {
          return res.json({ error: "Digite a senha do usuário" });
      }

      const validateMail = await AppDataSource.manager.findOneBy(User, { mail })

      if (validateMail) {
          return res.json({error: "O email digitado já está em uso"})
      } 
      ...
      if( idmaster ){
          ...
          if( !master || !master.iduser ){
            return res.json({ error: "Gestor não encontrado" });
          }
          if( master.profile !== "manager"){
            return res.json({ error: "O usuário digitado não possui perfil de gestor" });
          }
  ...
  public async update(req: Request, res: Response): Promise<Response> {
    ...
    if( !iduser ){
      return res.json({error:"Digite o id do usuário"});
    }
    ...
    if (user && user.iduser) {
      ...
      if(idmaster == iduser){
        return res.json({error:"O usuário não pode ser gerir a si proprio"});
      }
  ...
  public async delete(req: Request, res: Response): Promise<Response> {
    ...
    if( !iduser ){
      return res.json({error:"Digite o identificador do usuário"});
    }
  ...
  public async list(req: Request, res: Response): Promise<Response> {
    ...
    if (profile !== 'employee' && profile !== 'manager' && profile !== 'admin'){
      return res.json({ error: "Valor inválido para perfil. Digite employee, manager ou admin"})
    }
    ...
    if (department === 'false' || department === 'true') {
      ...
      return res.json({ error: "Valor inválido para departamento. Digite true ou false"})
    }
  ...
  public async updatePassword(req: Request, res: Response): Promise<Response> {
    ...
    if (!password || password.trim().length == 0) {
      return res.json({ error: "Digite a nova senha" });
    }
    ...
    if (user && user.iduser) {
      ...
      return res.json({ error: "Colaborador não encontrado" })
    }
  }
